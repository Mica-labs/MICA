...
class ConfigValidator(ABC):
    """Base class of validator for the Config file"""
    ...
    def validate_required_keys(
        self, content: Dict[Text, Any], required_keys: Set[Text]
    ) -> List[ValidationError]:
        errors = []
        for key in required_keys:
            if key not in content:
                errors.append(
                    ValidationError(
                        self.__class__.__name__, f"Missing required key '{key}'"
                    )
                )
        return errors

    def validate_spelling(self, content: Dict[Text, Any]) -> List[ValidationError]:
        errors = []
        for key in content:
    ...
    """
    def validate_type(self, content: Dict[str, Any]) -> List[ValidationError]:
        errors = []

        def _validate_value(
            value: Any, type_spec: TypeSpec, current_path: str
        ) -> List[ValidationError]:
            local_errors = []
            ...
            if isinstance(value, Dict) and type_spec.nested_schema:
                for key, nested_value in value.items():
                    if key in type_spec.nested_schema:
                        ...
                        local_errors.extend(
                            _validate_value(
                                nested_value, nested_spec, f"{current_path}.{key}"
                            )
                        )
        ...
        for key, value in content.items():
            if key in self.type_specs:
                errors.extend(
                    _validate_value(value, self.type_specs[key], f"{path}.{key}")
                )
...
class OpenAIValidator(ConfigValidator):
    """Validator for OpenAI Agent"""


class AnthropicValidator(ConfigValidator):
    """Validator for Anthropic Claude Agent"""

    def __init__(self):
        super().__init__()
        # add more keys
        self.required_keys = {"name", "anthropic-version", "max_tokens", "model"}
        self.valid_keys = {
            "name",
            "anthropic-version",
            "max_tokens",
        ...
        self.type_specs = {
            "name": TypeSpec(Text),
            "anthropic-version": TypeSpec(Text),
            "max_tokens": TypeSpec(Text),
            "model": TypeSpec(Text),
            "temperature": TypeSpec(Text),
            "top_k": TypeSpec(Text),
            "top_p": TypeSpec(Text),
        }
    ...
    def validate(
        ...
        errors.extend(self.validate_required_keys(content, self.required_keys))
        errors.extend(self.validate_spelling(content))
        # errors.extend(self.validate_type(content, path))

        return errors


class OpenRouterValidator(ConfigValidator):
    """Validator for OpenRouter Agent"""
...
class AgentValidator(ABC):
    ...
    def validate_required_keys(
        self, content: Dict[Text, Any], required_keys: Set[Text], path: Text
    ) -> List[ValidationError]:
        """验证必需的键"""
        errors = []
        for key in required_keys:
            if key not in content:
                errors.append(
                    ValidationError(
                        self.__class__.__name__, f"Missing required key '{key}'", path
                    )
                )
        return errors

    def validate_spelling(
        self, content: Dict[Text, Any], path: Text
    ) -> List[ValidationError]:
        """验证键的拼写"""
        errors = []
        for key in content:
    ...
    def validate_type(
        self, content: Dict[str, Any], path: str
    ) -> List[ValidationError]:
        """验证值的类型"""
        errors = []
    ...
    def validate_ref(
        self, agent_name: Text, path: Text, context: Dict[Text, Any], code_str=None
    ) -> List[ValidationError]:
        errors = []
        all_agents = context.get("all_agents", set())  # 获取所有已定义的agent名称
        if agent_name not in all_agents and (
            code_str is not None and agent_name not in code_str
        ):
            errors.append(
                ValidationError(
                    self.__class__.__name__,
                ...
                if step == "begin":
                    ...
                    if not in_condition:
                        if not found_begin:
                            errors.append(
                                ValidationError(
                                    "StepValidation", "Missing 'begin' in steps", path
                                )
                            )
                    ...
                    if key in ["if", "else if", "else"]:
                        if "then" not in step:
                            ...
                            if isinstance(step["then"], List):
                                errors.extend(
                                    self.validate_steps(
                                        step["then"], f"{step_path}/then", context, True
                                    )
                                )
                            else:
                                errors.append(
...
class FlowAgentValidator(AgentValidator):
    ...
    def validate(
        ...
        if "steps" in content:
            errors.extend(
                self.validate_steps(content["steps"], f"{path}/steps", context)
            )
...
class MainAgentValidator(AgentValidator):
    def __init__(self):
        ...
        self.type_specs = {"steps": TypeSpec(List)}
    ...
    def _validate_steps(self, content: List[Any], path: Text) -> List[ValidationError]:
        ...
        for i, step in enumerate(content):
            ...
            if "call" not in step:
                ...
                if "schedule" in step and step["schedule"] not in [
                    "priority",
                    "mediator",
                ]:
                    errors.append(
...
class Validator:
    ...
    def validate_config(self, llm_config: Dict) -> List[ValidationError]:
        """Checks if is yaml instance"""
        if not isinstance(llm_config, Dict):
            return [
                ValidationError("BasicValidation", f"Invalid config.yml structure.")
            ]
        ...
        llm_type = llm_config["name"]
        if llm_type not in self.config_validators:
            return [
                ValidationError("BasicValidation", f"LLM is not supported {llm_type}")
            ]

        copy_config = llm_config.copy()
        del llm_config["name"]

        validator = self.config_validators[llm_type]()
        return validator.validate(copy_config)

    def validate(
        self, agents_dict: Dict, code_str: Text = None
    ) -> List[ValidationError]:
        """验证YAML内容"""
        if not isinstance(agents_dict, Dict):
            return [
                ValidationError("BasicValidation", f"Invalid agents.yml structure.")
            ]
        ...
        # 创建验证上下文，包含所有agent名称
        context = {
            "all_agents": set(agents_dict.keys())
            - {"main", "tools"},  # 所有定义的agent名称
            "full_config": agents_dict,  # 完整的配置内容
        }

        # 验证每个代理
        for agent_name, agent_content in agents_dict.items():
            # print(f"agent_name: {agent_name}, agent_content: {agent_content}")
            if agent_name == "tools":
                continue
            if agent_name == "main":
                ...
                errors.extend(
                    validator.validate(
                        agent_content, f"agent[{agent_name}]", context, code_str
                    )
                )
                continue
