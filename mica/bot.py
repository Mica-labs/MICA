...
from mica.utils import (
    find_config_files,
    save_file,
    replace_args_in_string,
    logger,
    load_func_info_from_str,
    short_uuid,
)

from mica.llm.gemini_model import GeminiModel
from mica.llm.model import Model
from mica.llm.anthropic_model import AnthropicModel
...
class Bot(object):
    ...
    def __init__(
        ...
        self.sum_rsp_time = 0
        self.tools = tools
        self._func_args_config = {
            name: {} for name in self.tools.functions.keys()
        } or {}
        self._global_args = list(
            chain.from_iterable(
                a.contains_args() or []
                for a in agents.values()
                if isinstance(a, EnsembleAgent)
            )
        )
    ...
    def from_json(
        ...
        scheduler = scheduler_create.get(
            data.get("main").get("steps")[0].get("schedule") or "priority"
        )()
        ...
        if config.get("server") is not None:
            config["server"] = (
                config["server"] + "/rpc/rasa/message"
                if "openai" not in config["server"]
                and "anthropic" not in config["server"]
                else config["server"]
            )

        if "anthropic-version" in config:
            config["anthropic_version"] = config.pop("anthropic-version")

        llm_model = None
        llm_name = config.pop("name")
        if llm_name == "openai":
            llm_model = OpenAIModel.create(config)
        if llm_name == "anthropic":
            llm_model = AnthropicModel.create(config)
        if llm_model is None:
            raise ValueError("Must give a name for LLM")
        ...
        agents = {
            name: create_agents[value.get("type")](
                name=name, **value, **config, llm_model=llm_model
            )
                ...
                if agent.exit_agent is not None:
                    if isinstance(agent.exit_agent, Text):
                        ...
                        if exit_agent is None:
                            raise ValueError(
                                f"{name} fail to initialize: Exit agent {agent.exit_agent} not found"
                            )
                        agent.exit_agent = exit_agent
                    elif isinstance(agent.exit_agent, Dict):
                        exit_agent = DefaultExitAgent.create(
                            name=f"ExitAgent_{agent.name}",
                    ...
                    if isinstance(agent.fallback, Text):
                        if agent.fallback == "default":
                            fallback_agent = DefaultFallbackAgent.create(
                                name=f"DefaultFallbackAgent_{agent.name}",
                                llm_model=llm_model,
                            )
                        ...
                        if fallback_agent is None:
                            raise ValueError(
                                f"{name} fail to initialize: Fallback {agent.fallback} not found"
                            )
        ...
        if tool_code is not None:
            ...
            if load_rst["status"] == "success":
                ...
                logger.error(
                    f"Failed in loading python script. Error: {load_rst['error']}"
                )
    ...
    async def handle_message(
        self, user_id: Text, message: Any, channel: ChatChannel = None
    ):
    ...
    def _find_all_args(self, agents: Dict[Text, Agent]):
        all_args = {"sender": "", "bot_name": self.name}
        if agents is None:
            return all_args
