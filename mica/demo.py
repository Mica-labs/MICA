...
async def generate_bot(bot_name, llm_config, yaml_input, code, chat_url, user_id):
    try:
        parsed_config = yaml.safe_load(llm_config)
        llm_name = parsed_config.get("name")
        parsed_yaml = yaml.safe_load(yaml_input)
        # validate
        validator = Validator()
        result_config = validator.validate_config(parsed_config)
        assert result_config == [], "Did not pass the validation"
        result = validator.validate(parsed_yaml, code)
        assert result == [], "Did not pass the validation."
        # convert
        parsed_config["server"] = chat_url
        parsed_config["name"] = llm_name
        parsed_agents = parser.parse_agents(parsed_yaml)
        # print("HELLO:", parsed_config)
        bot = Bot.from_json(
            name=bot_name, data=parsed_agents, config=parsed_config, tool_code=code
        )
        ...
        msgs = [
            f"Error Type: {err.rule_name}, Message: {err.message}"
            for err in result_config
        ]
        msgs_str = "\n".join(msgs)
        raise gr.Error(
            f"Did not pass the validation. "
    ...
    if main.steps:
        for step in main.steps:
            if isinstance(step, BotStep):
                _, chatbot, user_id, tracker = await get_response(
                    "/init", chatbot, bot, user_id
                )
            if isinstance(step, Call):
                ...
                if isinstance(initial_agent, FlowAgent):
                    for step in initial_agent.subflows[
                        initial_agent.main_flow_name
                    ].steps:
                        if isinstance(step, BotStep):
                            _, chatbot, user_id, tracker = await get_response(
                                "/init", chatbot, bot, user_id
...
async def load_bot(files, chatbot, user_id):
    ...
    try:
        ...
        bot, chatbot, user_id, tracker = await generate_bot(
            bot_name, agents, tools, user_id
        )
...
if __name__ == "__main__":
    with gr.Blocks(theme=gr.themes.Base()) as demo:
        with gr.Row():
            with gr.Column():
                chat_url = gr.Textbox(label="Enter chat url", lines=1, value=None)
                llm_config = gr.Code(
                    value=None,
                    label="Enter config.yml",
                    language="yaml",
                    lines=10,
                    interactive=True,
                )
                bot_name = gr.Textbox(
                    label="Enter Bot Name", lines=1, value="Default bot"
                )
                yaml_input = gr.Code(
                    value="""book_restaurant:
  type: llm agent
...
main:
  ...
  - call: meta
    schedule: priority
""",
                    label="Enter agents.yml",
                    language="yaml",
                    lines=15,
                )
                code_input = gr.Code(
                    label="Enter tools.py", language="python", lines=10, value=None
                )
                bot = gr.State(None)

            with gr.Column():
                file_loader = gr.FileExplorer(
                    root_dir="./examples", glob="**/*.*", label="Open"
                )
            ...
            msg.submit(
                get_response,
                [msg, chatbot, bot, user_id],
                [msg, chatbot, user_id, tracker],
            )
            submit_btn.click(
                generate_bot,
                [bot_name, llm_config, yaml_input, code_input, chat_url, user_id],
                [bot, chatbot, user_id, tracker],
            )
            save_btn.click(save_bot, [bot_name, yaml_input, code_input])
            file_loader.change(
                load_bot,
                inputs=[file_loader, chatbot, user_id],
                outputs=[
                    bot,
                    bot_name,
                    yaml_input,
