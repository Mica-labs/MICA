    ...
    async def run(self,
        ...
        if next_agent is not None:
            if next_agent in self.contains:
                ...
                logger.info(f"Ensemble agent: [{self.name}] selected next agent: {next_agent}")
                ...
                logger.info(f"Ensemble agent: [{self.name}] output response: {ensemble_response}")
        ...
        if tracker.events[-1] == tracker.latest_message:
            ...
            is_end = True
            logger.info(f"Ensemble agent: [{self.name}] selected fallback agent and get response: {fallback_response}")
            return is_end, agent_result
        ...
        if self.exit_agent is not None and len(agent_result) == 0:
            ...
            logger.info(f"Ensemble agent: [{self.name}] selected exit agent and get response: {exit_response}")
        return True, agent_result
