...
class LLMAgent(Agent):
    ...
    async def run(self, tracker: Tracker, is_tool=False, **kwargs):
        ...
        logger.debug(
            "LLM agent functions prompt: \n%s",
            json.dumps(functions, indent=2, ensure_ascii=False),
        )
        llm_result = await self.llm_model.generate_message(
            prompt, functions=functions, tracker=tracker, provider=self.name
        )
        is_end = True
        final_result = []
        ...
        for event in llm_result:
            if isinstance(event, FunctionCall):
                ...
                logger.debug(
                    f"Execute function: {event.function_name}, get result: {tool_rst}"
                )
                ...
                if tool_rst["result"] is not None:
                    ...
                    if isinstance(tool_rst_states, Text):
                        final_result.append(
                            BotUtter(tool_rst_states, provider=self.name)
                        )
                        ...
                        for evt in tool_rst_states:
                            ...
                            if evt.get("slot_name") is not None:
                                ...
                                tracker.set_arg(
                                    slot_info.get("flow_name"),
                                    slot_info.get("arg_name"),
                                    setslot.value,
                                )
                ...
                if tool_rst["stdout"] is not None and len(tool_rst["stdout"]) > 0:
                    ...
                    is_end, sec_call_result = await self.run(
                        tracker, is_tool=True, **kwargs
                    )
            ...
            if isinstance(event, BotUtter):
                try:
                    try:
                        ...
                        json_starts = [
                            m.start() for m in re.finditer(r"\{", event.text)
                        ]
                        ...
                        for start in json_starts:
                            ...
                            raise json.JSONDecodeError(
                                "No valid JSON object found", event.text, 0
                            )

                except json.JSONDecodeError as e:
                    logger.error(f"JSON extraction failed: {e}. Text: {event.text}")
                    response = {"bot": event.text}
                if len(llm_result) == 1:
                    tracker.set_conv_history(
                        self.name, {"role": "assistant", "content": event.text}
                    )
        ...
        if not is_tool:
            latest_user = {
                ...
                "content": (
                    ...
                    if self._is_interrupted(tracker)
                    else tracker.latest_message.text
                ),
            }
    ...
    def _is_interrupted(self, tracker: Tracker) -> bool:
        if len(tracker.get_or_create_agent_conv_history(self.name)) == 0:
            return False
        last_agent_response = safe_json_loads(
            tracker.get_or_create_agent_conv_history(self.name)[-1].get("content")
        ).get("bot")
        last_bot_response = ""
    ...
    def _generate_function_prompt(
        self, tools: Optional[Any] = None, **kwargs
    ) -> Union[List, None]:
