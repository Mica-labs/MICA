...
class KBAgent(Agent):
    def __init__(
        ...
        **kwargs,
        ...
        self.prepare(
            knowledge_base["faq"], knowledge_base["file"], knowledge_base["web"]
        )
    ...
    def create(
        ...
        **kwargs,
        ...
        knowledge_base = {"faq": faq, "file": file, "web": web}
        return cls(name, description, config, knowledge_base, llm_model=llm_model)
    ...
    def prepare(
        ...
        if faq_data:
            for question, answer in faq_data.items():
                ...
                documents.append(
                    Document(
                        page_content=content, metadata={"source": "faq", "type": "qa"}
                    )
                )
        ...
        if documents:
            ...
            logger.debug(
                f"Indexed {len(texts)} text chunks from {len(documents)} documents"
            )
